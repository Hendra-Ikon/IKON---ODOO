...
class CrmSaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        id = self[0].id
        datesale = self.env['sale.order.line'].search([('order_id','=', id)])
        datas =  []
        for sale in datesale:
            sale_data = {
                'item_id': sale.item_id,
                'item_description': sale.item_description,
                'period': sale.period,
            }
            datas.append(sale_data)
        logger.info("yee",datas)
        ...
        if not grouped:
            ...
            logger.info("new_invoice_vals_list",new_invoice_vals_list)
